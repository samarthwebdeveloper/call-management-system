...
export class AttendanceService {
    ...
    public Save = (data: any): Observable<any> => {
        let body = this.serializeObj(data);
        return this.http.post(this.configuration.Server + 'AttendanceLog/Save',  body, this.options)
               .map(res => <any>res.json());
    }

    public GetAll = (data: any): Observable<any> => {
        let body = this.serializeObj(data);
        return this.http.post(this.configuration.Server + 'AttendanceLog/GetList',  body, this.options)
               .map(res => <any>res.json());
    }
